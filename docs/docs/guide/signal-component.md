...
使用`$((state)=>{.....})`创建一个组合信号组件，传入一个函数，当函数内部依赖的的状态数据变化时，该信号组件会自动触发局部更新。
...
- 会自动返回该订阅异步计算属性的`value`值
- 会自动订阅异步计算属性的`value`变更事件
...
:::warning{title=自定义渲染}
以上的信号组件仅仅是将状态数据渲染为`ReactNode`，我们也可以自定义渲染，提供一个`render`函数，让开发者自定义渲染逻辑，在状态变化时重新执行渲染。
:::
...
:::waring{title=注意🌝}
以上的信号组件均是基于`State`中已经声明的`computed`计算属性创建的。我们也支持动态创建一个计算信号组件。
:::
