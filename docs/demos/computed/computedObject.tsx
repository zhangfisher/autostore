...
export default () => {
    ...
    return (
        <div>
            ...
            <Box>
                <RichLabel
                    text={`typeof(store.computedObjects)=={${typeof store.computedObjects}}`}
                />
                <RichLabel
                    text={`store.computedObjects instanceof Map={${String(
                        store.computedObjects instanceof Map,
                    )}}`}
                />
                <RichLabel text={`store.computedObjects.size={${store.computedObjects.size}}`} />
                <RichLabel
                    text={`store.computedObjects.keys()={${[...store.computedObjects.keys()].join(
                        ' , ',
                    )}}`}
                />
                <Button onClick={() => store.computedObjects.get('user.fullName')!.run()}>
                    手动执行fullName计算函数
                </Button>
                <Button onClick={() => store.computedObjects.get('user.fullName2')!.run()}>
                    手动执行fullName2计算函数
                </Button>
                <Button onClick={() => store.state.user.fullName2.run()}>
                    手动执行fullName2计算函数
                </Button>
            </Box>
        </div>
