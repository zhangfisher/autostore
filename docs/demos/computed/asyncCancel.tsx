...
export default () => {
    ...
    return (
        <div>
                    ...
                    [
                        () => {
                            return (
                                <>
                                    {total.loading ? (
                                        <RichLabel inline text={`正在计算......`} />
                                    ) : (
                                        total.error && (
                                            <RichLabel inline text={`ERROR: {${total.error}}`} />
                                        )
                                    )}
                                    {total.loading ? (
                                        <Button onClick={() => total.cancel()}>取消</Button>
                                    ) : null}
                                </>
                            );
                        },
            ...
            />
            <JsonView title="state.order.total=" highlightKeys={['error', 'value']} data={total} />
        </div>
    );
};
