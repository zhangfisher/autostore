...
| 配置项 | 形式 | 说明 |
| --- | --- | --- |
| `.keep` | 修饰符 | 仅切 display、保留子树与 watcher（`x-show` 是其别名） |
| `keep` | `x-if-options="{keep:true}"` | 与 `.keep` 等价 |

| 元数据 | 值 | 说明 |
| --- | --- | --- |
| `priority` | `80` | 结构指令，先于绑定/内容指令 |
| `singleton` | `true` | 同元素同名取最后声明 |
| `ownsChildren` | eager 为 `true`，keep 为 `false` | 决定是否独占子树 |
