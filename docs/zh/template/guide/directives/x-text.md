...
`x-text` 的指令值是绑定的表达式；额外的空值行为经指令选项与修饰符配置。

**指令选项**（`x-text-options`，宽松 JSON 对象，可经 `x-options` 元素级回退）：

| 选项          | 类型     | 默认                 | 说明                                                                       |
| ------------- | -------- | -------------------- | -------------------------------------------------------------------------- |
| `empty`       | `string` | `""`                 | 值为空时渲染的占位内容（静态字面量，不响应式）                             |
| `emptyValues` | `any[]`  | `[]`（追加到默认集） | 追加「也算空」的值，如 `[0, ""]`；默认集 `[null, undefined, NaN]` 始终兜底 |

**修饰符**：

| 修饰符  | 说明                                                                                    |
| ------- | --------------------------------------------------------------------------------------- |
| `.hide` | 值为空时隐藏宿主元素（`display:none`，恢复时还原原内联 display）；与 `empty` 并存时优先 |

| 元数据      | 值     | 说明                                                                     |
| ----------- | ------ | ------------------------------------------------------------------------ |
| `priority`  | `0`    | 内容类指令，优先级低于结构指令（`x-for`/`x-if`）与绑定指令               |
| `singleton` | `true` | 同一元素同名只保留最后一条声明，如 `<span x-text="a" x-text="b">` 取 `b` |
