...
type StateOperate<Value = any, Parent = any> = {
    ...
    path: string[];
    value: Value;
    // 数组操作时，操作的索引，如[1,2]表示操作了数组的第1个和第2个元素
    indexs?: number[];
    oldValue?: Value;
    parentPath?: string[];
    parent?: Parent;
...
|   `flags`    | `boolean`  | 扩展标识,可以在update时传递，在监听器函数中读                             |

- `watch`状态的读写操作,包括`get`,`set`,`delete`,`insert`,`update`,`remove`,`batch`等操作进行监听。
- `get`,`set`,`delete`适于对象的值的读写
- `insert`,`update`,`remove`适于数组的操作
- `batch`：适于批量操作,当使用`batchUpdate`会触发此类型的操作事件
- `reply`： 参数用于标识该操作是否是在批量更新时的事件回放，参阅批量更新。
- `flags`: 可以在`store.update`时传入，然后在监听器函数中读取到该值，用于对更新进行一些额外的控制标识

:::warning 注意
监听函数只能是一个同步函数,默认只监听写操作。
:::
