...
`AutoStore`提供了非常强大的异步计算属性特性，使用`computed`和`asyncComputed`来声明创建一个异步计算属性。

:::warning 提示

- `asyncComputed(async (scope)=>{....})`声明的异步计算属性均会被原地替换为`AsyncComputedValue`对象。
- `computed(async (scope)=>{....})`声明的异步计算属性只会写入计算结果。
  :::
