...
- **默认 toView 特例**（widget 显式声明才启用，不做值格式启发）：`widget='color'` 渲染宽 3em×高 1em 条形色块（圆角 4px、padding 4px、border 1px、title=值），非颜色格式值回落文本；`widget='checkbox'` 渲染 disabled（不灰化）只读勾选框，勾选态复用双值档位判定。disabled 控件不派发鼠标事件，双击进编辑改用工具按钮。
