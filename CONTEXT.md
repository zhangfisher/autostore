...
**Shallow 浅响应（Shallow）**:
`shallow()` 标记的对象值进入状态树后仅创建一层浅代理：顶层的读写与数组操作照常产生操作事件，但属性值不再递归代理——读出即为原始引用，直接修改其内部不产生任何事件。标记只作用于被标记对象本身，标记经内部 Symbol 键挂载，不泄漏进序列化。用于大数组/大对象的性能特区。代理深度由此三分：深代理（默认）/ 浅代理（shallow）/ 不代理（markRaw）。
_Avoid_: 非响应式（那是 markRaw）、局部响应式、部分代理
