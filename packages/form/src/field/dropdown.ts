...
export type AutoDropdownFieldOptions = {
    dropdown?: boolean;
    renderSelection?: (value: any, html: any) => string;
};

export class AutoDropdownField<Options = unknown> extends AutoField<
    Options & AutoDropdownFieldOptions
> {
    static styles = [
        AutoField.styles,
        css`
    ...
    _renderSelection() {
        return html`<div class="selection" slot="trigger">
            ...
            ${when(
                this._isEmpty() && this.options.placeholder,
                () => html`<span class="placeholder">${this.options.placeholder}</span>`,
                () => {
                    return html`<span class="select-value"> ${this.renderSelection()} </span>`;
                },
            )}
            <span class="suffix">
                <sl-icon
    ...
    _renderContent() {
        return html`<div
            class="popoup-container ${ifDefined(this.options.dropdown ? "dropdown" : undefined)}"
        >
            ${this.renderDropdown()}
        </div>`;
    }
    ...
    renderSelection(val?: any) {
        return html`
            ${this.options.renderSelection
                ? this.options.renderSelection(val || this.value, html)
                : val || this.value}
        `;
    }
    renderInput() {
