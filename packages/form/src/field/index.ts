...
export class AutoField<Options = unknown> extends LitElement {
	...
	isShowError() {
		if (this.context.validAtInit) {
			...
			// 不显示
			return this.dirty ? !!this.invalidTips : false;
		}
	}
	renderError() {
		return this.isShowError() ? html`<div class="error">${this.invalidTips}</div>` : html``;
	}
	...
	updateOptions() {
		...
		if (ctx?.store && this.schema) {
			...
			if (this.path in ctx.store.configManager.errors) {
				this.invalidTips = ctx.store.configManager.errors[this.path];
			}
			if (Array.isArray(this.options.actions)) {
				this.beforeActions = this.options.actions.filter((action: any) => (action as any).pos === "before");
				this.afterActions = this.options.actions.filter((action: any) => (action as any).pos !== "before");
			}
		}
	}
	...
	_updateFormClasss() {
		...
		applyClass(this.context.form, "invalid", !!this.invalidTips);
	}
	/**
	 * 当输入框值改变时更新状态
	 * @returns
	...
	renderValue() {
		return html`
            ${this.renderInput()} ${when(this.context.viewonly, () => this.renderHelp())}
            ${this.renderError()}
        `;
	}
	getPath(): string[] {
...
updated(changedProperties: Map<string, any>) {
	...
	render() {
		const ctx = this.context;
		const labelPos = (this.options as any).labelPos ? (this.options as any).labelPos : ctx.labelPos;
