...
export class AutoFieldUpload extends AutoField<AutoFieldUploadOptions> {
	...
	_getDefaultFileLabel(file: string | any) {
	...
	_createUploadInput() {
		...
		if (this.options.fileTypes.length > 0) {
			...
			this.fileInputRef.accept = this.options.fileTypes.filter((type: any) => type !== "*").join(",");
		}
	...
	private handleDrop(e: DragEvent) {
		...
		if (this.options?.fileTypes && this.options.fileTypes.length > 0) {
			const invalidFiles = files.filter((file) => {
				...
				return !this.options.fileTypes.some((type: any) => {
					if (type === "*") return true;
					if (type.startsWith(".")) {
						// 检查文件扩展名
	...
	renderProgressbar(file: UploadFile, dir: "hori" | "vert") {
		...
		const style =
			dir === "hori" ? `width:${file.progress}%;` : `height:${file.progress}%;top:${100 - file.progress}%`;
		return html`<span
	...
	renderInput() {
		return html`
            <auto-flex grow="none" gap="0.5rem" direction="column">
                ${this.renderFiels()}
                ${when(
					this.options.selector === "rectangle" ||
						(this.options.selector === "auto" && this.options.multiple),
					() => {
						return html`<div
                        class="indicator"
