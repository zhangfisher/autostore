...
import { VALUE_SCHEMA_BUILDER_FLAG } from '../../core/src/consts';
import type { AutoStore } from '../../core/src/store';
import type {
    ComputedState,
    Dict,
    GetTypeByPath,
...
export type SchemaValidate<Value = any> = (
    newValue: Value,
    oldValue: Value,
    path: string[],
) => boolean;
...
export type ComputedSchemaState<State extends Dict> = {
    [Key in SchemaKeyPaths<State>]: (GetTypeByPath<State, Key> extends SchemaDescriptorBuilder<
        infer V,
        infer Options
    >
        ? Options & { value: V }
