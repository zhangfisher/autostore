...
export function parsePadding(padding: string): { left: number, right: number, top: number, bottom: number, unit: string } {
    // 默认值
    const result = {
        left: 0,
        right: 0,
 ...
 */
export function toPadding(values: string[]): string {
