...
class IPConfigSyncForm extends LitElement {
    ...
    private _syncSettings() {
        ...
        const filter = (path: string[]) => {
            ...
            // 检查 configManager.state 中是否存在该路径的 schema
            const fullPath = path.join('.');
            return !!configManager.state[fullPath];
        };
        ...
        const syncSettings = {
            ...
            pathMap: {
                ...
                toRemote(path: any[], value: any) {
                    // 检查是否是 configurable 字段
                    // 当同步configurable时，如果是数组或对象，也需要同步
                    const configManager = moduleStore.configManager;
                    const hasSchema = configManager && configManager.state[path.join('.')];
                    if (typeof value !== 'object' || hasSchema) {
                        return [path.join('.')];
                    }
                },
            },
