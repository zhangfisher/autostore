...
> =
	...
	| (DefaultKind extends never ? never : Union<{ [K in KindKey]?: DefaultKind } & Items[DefaultKind] & Share>);

export type PickValues<T extends Record<string, any>> = Union<UnionToIntersection<T[keyof T]>>;

// **************  以下实现将计算属性函数的返回值类型提取出来  **************

export type PickComputedResult<T> = T extends SchemaDescriptorBuilder<infer X, any>
	? X
