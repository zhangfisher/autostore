...
export class ObserverObject<Value = any, Options extends ObserverOptions<Value> = ObserverOptions<Value>> {
	...
	attach() {
		if (!this._attached && this.depends && this.depends.length > 0) {
			this._subscribers.push(
				this.shadowStore.watch(this.getDepends(), this.onDependsChange.bind(this), { operates: "write" }),
			);
			this.store.log(
				() =>
					`${this.toString()} subscribed to ${this.depends!.map((depends) => depends.join(PATH_DELIMITER)).join(",")}`,
			);
			this._attached = true;
		}
	}
