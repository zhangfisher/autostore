...
export class AutoStore<State extends Dict, Options = unknown> extends FastEvent<StoreEvents> {
	...
	private _createSandbox() {
		if (this.options.enableValueExpr) {
			const sandbox = isFunction(this.options.sandbox?.create) ? this.options.sandbox.create : createSandbox;
	...
	private _onFirstEachState({ value, path }: { path: string[]; key: string; value: any; parent: any }) {
	...
	private subscribeCallbacks() {
		if (this.options.onComputedCreated) this.on("computed:created", this.options.onComputedCreated.bind(this));
		if (this.options.onComputedDone) this.on("computed:done", this.options.onComputedDone.bind(this));
		if (this.options.onComputedError) this.on("computed:error", this.options.onComputedError.bind(this));
		if (this.options.onComputedCancel) this.on("computed:cancel", this.options.onComputedCancel.bind(this));
		if (this.options.onObserverBeforeCreate)
			this.on("observer:beforeCreate", this.options.onObserverBeforeCreate.bind(this));
		if (this.options.onObserverCreated) this.on("observer:created", this.options.onObserverCreated.bind(this));
	}
	/**
	 *
	 * 当状态读写时调用此方法触发事件
	...
	_createWatch(descriptor: WatchDescriptor, computedContext?: ComputedContext) {
		this.emit("observer:beforeCreate", descriptor as ObserverDescriptor<any, any, any, any, any>);
	...
	update(fn: (state: ComputedState<State>) => void, options?: UpdateOptions) {
		const { batch = false, reply = true, silent = false, peep = false, flags = 0, validate } = options || {};
		...
		const getter =
			typeof arguments[0] === "function"
				...
				: () =>
						getVal(
							this.state,
							Array.isArray(arguments[0]) ? arguments[0] : arguments[0].split(this.delimiter),
						);
		this._peeping = true;
		try {
			return getter();
	 ...
	 *  @returns
	 */
	getSnap<Entry extends string>(options?: { entry?: Entry; reserveAsync?: boolean; includeFunc?: boolean }) {
