...
import { forEachObject, getSnapshot, getVal, isAsyncComputedValue, isPathEq, markRaw, setVal } from "../utils";
...
export class AutoStore<State extends Dict> extends EventEmitter<StoreEvents> {
	...
	private createObserverObject(path: string[], value: any, parentPath: string[], parent: any) {
		if (isSchemaBuilder(value)) {
			const schema = this.schemas.add(path, value);
			return schema.value;
		} else {
		...
		const getter =
			typeof arguments[0] === "function"
				...
				: () =>
						getVal(
							this.state,
							Array.isArray(arguments[0]) ? arguments[0] : arguments[0].split(this._delimiter),
						);
		this._peeping = true;
		try {
			return getter();
