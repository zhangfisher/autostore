...
export function hookArrayMethods(notifyChange:any, array: any[], name:string,method:Function, parentPath: string[]) {
    if(name==='push'){
        ...
        return (start: number, deleteCount: number, ...items: any[]) => {
            const deletedItems = method.apply(array, [start, deleteCount, ...items]);                
        ...
        return (value: any, start?: number, end?: number) => { 
            ...
            const affectedIndexes = Array.from({ length: endIndex - startIndex }, (_, i) => i + startIndex);
            const affectedValues = Array.from({ length: endIndex - startIndex }, () => value);
            notifyChange({type:'update',path:parentPath,indexs:affectedIndexes,value:affectedValues,oldValue:undefined, parentPath,parent:array});
            return result;
        }
    }else if(name==='concat'){
        return (...items: any[]) => {
            ...
            notifyChange({type:'insert',path:parentPath,indexs:affectedIndexes,value:items,oldValue:undefined, parentPath,parent:array});
            return result;
        }
    }else{
        return method
