...
import type { Watcher } from "../../src/watch/types";
    ...
    describe("watch object", () => {
        ...
        test("Write operage events", () => {
            ...
            expect(events[0]).toStrictEqual({
                type: 'set', path: ['firstName'], indexs: [], value: "Li", parent: store.state, parentPath: [], oldValue: 'John',
                flags: 0
            })
            expect(events[1]).toStrictEqual({
                type: 'set', path: ['address', '0', 'city'], indexs: [], value: "Shanghai", parent: store.state.address[0], parentPath: ["address", '0'], oldValue: 'New York',
                flags: 0
            })
            expect(events[2]).toStrictEqual({
                type: 'set', path: ['job', 'title'], indexs: [], value: "Frontend Engineer", parent: store.state.job, parentPath: ["job"], oldValue: 'Software Engineer',
                flags: 0
            })
        })
        test("delete object items", () => {
            ...
            expect(events[0]).toStrictEqual({
                type: 'delete', path: ['firstName'], indexs: [], value: 'John', parent: store.state, parentPath: [], oldValue: undefined,
                flags: 0
            })
            expect(events[1]).toStrictEqual({
                type: 'delete', path: ['address', '0', 'city'], indexs: [], value: 'New York', parent: store.state.address[0], parentPath: ["address", '0'], oldValue: undefined,
                flags: 0
            })
            expect(events[2]).toStrictEqual({
                type: 'delete', path: ['job', 'title'], indexs: [], value: 'Software Engineer', parent: {
                    company: 'Google',
                    salary: 100000
                }, parentPath: ["job"], oldValue: undefined,
                flags: 0
            })
        })
