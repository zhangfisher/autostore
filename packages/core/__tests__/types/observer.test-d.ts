...
describe("Observer 基础类型", () => {
    ...
    test("ObserverScope 类型", () => {
        ...
        type Case = Expect<
            Equal<Scope, string | string[] | "SELF" | "CURRENT" | "ROOT" | "PARENT">
        >;
    });
...
describe("ObserverOptions 类型", () => {
    ...
    test("ObserverOptions 属性类型", () => {
        ...
        type Case3 = Expect<
            Equal<ScopeType, string | string[] | "SELF" | "CURRENT" | "ROOT" | "PARENT" | undefined>
        >;
...
describe("ObserverDescriptor 类型", () => {
    test("基础 ObserverDescriptor", () => {
        ...
        type Case = Expect<Equal<keyof Descriptor, "type" | "getter" | "options">>;
    });
...
describe("ObserverObject 类型", () => {
    ...
    test("ObserverObject.watch 方法", () => {
        ...
        if (observerObject) {
            ...
            type Case = Expect<
                Equal<keyof WatcherType, "off" | "pause" | "resume" | "isWatching" | "watch" | "id">
            >;
        }
    });
...
describe("Observer 描述符构建器", () => {
    test("computed 描述符构建器", () => {
        ...
        // 描述符构建器应该有这些方法
        type Case = Expect<
            Equal<keyof DescriptorBuilder, "type" | "getter" | "depends" | "options" | "build">
        >;
    });
