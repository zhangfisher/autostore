...
/**
 * 兼容 core 旧 API 的辅助函数：在已安装 shadow 插件的 store 上创建 shadowStore。
 * 等价于 store.shadow(state, options)
 */
function createShadow<T extends Dict>(
    store: AutoStore<any>,
    state: T,
    options?: AutoStoreOptions<T>,
) {
    return store.shadow(state, options);
}
...
describe("Shadow Store基本功能", () => {
    ...
    test("Shadow store的ID基于原store", () => {
        const store = new AutoStore({ price: 10 }, { plugins: [shadow] });
        const shadowStore = store.shadow({
