...
<div>I am {{name}}</div>
<!-- 变量 -->
<div>I am {{ firstName + lastName }}</div>
<!-- 表达式：任一依赖变化即重算 -->
<!-- 属性值 -->
...
<!-- 混合，走 property -->
<button disabled="{{isLocked}}">提交</button>
<!-- 整体单段，走 boolean -->
```
...
- **`{{ obj.field }}`**（obj 是响应式对象引用，如 x-for 项 `user` 指向 `state.users[i]`）→ `getContext().obj` 返回响应式代理，`.field` 读取经代理 → `collectDependencies` 收集 → **细粒度响应式**。
