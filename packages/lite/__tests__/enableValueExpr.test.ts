...
describe("异步计算属性测试", () => {
    ...
    test("进度条应该与 retry 选项配合使用", async () => {
        ...
        const store = new AutoStore(
            {
                shouldFail: true,
                value: '```asyncComputed(async (scope,{getProgressbar})=>{data.attemptCount++;const pbar = getProgressbar({max:100,min:0});pbar.value(data.attemptCount*25);if(data.attemptCount<=2){throw new Error("fail")}await new Promise(r=>setTimeout(r,10));pbar.end();return "success"},[],{retry:3})```',
            },
            { sandbox: { context: sandbox } },
        );
