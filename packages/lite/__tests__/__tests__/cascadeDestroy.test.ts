...
describe("cascadeDestroy 级联销毁观察对象", () => {
    ...
    test("全局 cascadeDestroy=false 时不会自动销毁", () => {
        const store = new AutoStore(
            {
                a: 1,
                c: (scope: any) => scope.a,
    ...
    test("单个观察对象 cascadeDestroy=false 覆盖全局", () => {
        const store = new AutoStore({
            a: 1,
            d: 2,
            // c 关闭自动销毁
    ...
    test("onObserverDestroyed 回调与 observer:destroyed 事件均触发一次", () => {
        let cbCount = 0;
        let evtCount = 0;
        const store = new AutoStore(
