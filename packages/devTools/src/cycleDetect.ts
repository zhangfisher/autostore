...
export type CycleDetectPluginOptions = {
    /**
     * 当检测到多少个循环时视为循环依赖
     */
    cycleCount?: number;
...
export class AsyncCycleDetector {
    ...
    start() {
        this.running = true;
        this.steps.push(this.path);
        this._watcher = this.computedObject.store.watch(
            (event: any) => {
    ...
    private analysisCycleSteps() {
        ...
        this.computedObject.store.log(
            `CycleDepend detected in <${this.computedObject.toString()}>: ${paths}`,
            "warn",
        );
        if (typeof onDetected === "function") {
            ...
            if (result === "ignore") {
                ...
                throw new Error(
                    `CycleDepend found in <${this.computedObject.toString()}>: ${paths}`,
                );
            }
        }
    }
...
function createCycleDetectPlugin(options?: CycleDetectPluginOptions) {
    return function cycleDetectPlugin(store: any) {
        ...
        if (typeof onObserverCreated === "function") {
            ...
            store.log(
                `Store<${store.id}> onObserverCreated is already configured, cycleDetectExtend is disabled`,
                "warn",
            );
        } else {
...
export function installCycleDetectExtend(options?: CycleDetectPluginOptions) {
    // @ts-ignore
    ...
    // @ts-ignore
    globalThis.__AUTOSTORE_PLUGINS__.push(createCycleDetectPlugin(options));
}
