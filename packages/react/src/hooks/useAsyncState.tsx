import { AsyncComputedValue, isAsyncComputedValue, isPlainObject, PATH_DELIMITER, setVal, Watcher,  type Dict } from "autostore";
...
export function createUseAsyncState<State extends Dict>(store:ReactAutoStore<State>){
    return  (function(){
        ...
        const [ result ] = useState<AsyncComputedValue>(()=>{
            const val = getValueBySelector(store,selector)
            if(isAsyncComputedValue(val)){
                return val
            }else{
                return {value:val,loading:false,retry:0,timeout:0,progress:0,run:()=>{},cancel:()=>{}}
            }
        })    
