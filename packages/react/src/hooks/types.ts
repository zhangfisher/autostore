...
// ********** useState **********
// [值，修改值的函数，异步加载状态，错误]
export type UseStateResult<Value, State extends Dict> = [
    Value extends Dict ? ComputedState<Value> : Value,
...
export type UseStateSetter<SetValue, State extends Dict> = (
    value: SetValue,
    state: ComputedState<State>,
) => void;
export type UseStateComposeResult<Value, SetValue, State extends Dict> = [
    Value extends Dict ? ComputedState<Value> : Value,
...
export interface UseStateType<State extends Dict> {
    <Path extends StatePaths<State> = StatePaths<State>>(
        selector: Path,
    ): UseStateResult<GetTypeByPath<State, Path>, State>;
    <Path extends StatePaths<State> = StatePaths<State>>(
        selector: Path,
        async: boolean,
    ): UseStateResult<AsyncComputedValue<GetTypeByPath<State, Path>>, State>;
    <Value = any>(selector: string[]): UseStateResult<Value, State>;
    <Value = any>(
        selector: string[],
        async: boolean,
    ): UseStateResult<AsyncComputedValue<Value>, State>;
    <Value = any, SetValue = any>(
        getter: UseStateGetter<Value, State>,
        setter?: UseStateSetter<SetValue, State>,
    ): UseStateComposeResult<Value, SetValue, State>;
...
export interface UseReactiveType<State extends Dict> {
    ...
    <Path extends StatePaths<State> = StatePaths<State>>(
        selector: Path,
        async: boolean,
    ): UseReactiveResult<AsyncComputedValue<GetTypeByPath<State, Path>>, State>;
